...
const TestimonialsSectionExactyMed = () => {
  ...
  const marqueeRowRef = React.useRef<HTMLDivElement | null>(null);
  ...
  const resumeMarquee = React.useCallback(() => {
    const row = marqueeRowRef.current;
    if (!row || prefersReducedMotion) return;
    // Resume from the same offset — only when the CSS marquee is active.
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-[1440px] px-6 md:px-8">
        ...
        >
          <div
            className="testimonials-marquee-shell relative overflow-hidden py-3"
