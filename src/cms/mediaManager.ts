  ...
  try {
    ...
    throw new Error(
      `Upload failed: API Express indisponível em /api/assets/upload (status ${res.status}). ` +
        `Em local: \`npm run dev\` (Vite + API). Em Hostinger Node: \`npm run build:hostinger\` e start \`npm run start:hostinger\`.`,
    );
  }
...
export const loadPersistedMediaAssets = (): Array<Record<string, unknown>> => {
  try {
    ...
    console.error(
      "[Exacty CMS] Não foi possível carregar a biblioteca de mídia (API indisponível ou resposta não JSON).",
      error,
    );
    return [];
  }
